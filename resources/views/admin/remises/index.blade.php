...
@section('title', 'Gestion des remises (Admin)')
...
<div class="py-12">
    <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
        ...
        <div class="flex flex-col md:flex-row md:items-center md:justify-between gap-4 mb-10 animate-fade-in-down">
            <div>
                <h1 class="text-4xl font-extrabold bg-gradient-to-tr from-pink-600 via-red-500 to-yellow-400 bg-clip-text text-transparent tracking-tight drop-shadow-lg">Gestion des Remises (Admin)</h1>
                <p class="mt-2 text-lg text-gray-500">Visualisez, filtrez et gérez toutes les remises disponibles pour vos clients.</p>
            </div>
            <div class="flex flex-wrap gap-4">
                <a href="{{ route('admin.remises.create') }}" class="inline-flex items-center gap-2 px-6 py-3 rounded-xl bg-gradient-to-tr from-pink-600 to-yellow-400 text-white font-bold shadow-xl hover:shadow-neon hover:-translate-y-1 transition-all duration-200">
        ...
        <div class="bg-white/60 backdrop-blur-xl border border-pink-100 rounded-2xl shadow-2xl p-6 mb-8">
            <form action="{{ route('admin.remises.index') }}" method="GET" class="flex flex-wrap gap-4 items-center">
            ...
            <table class="min-w-full divide-y divide-pink-100">
                ...
                <tbody class="bg-white/60 divide-y divide-pink-50">
                    ...
                    <tr class="hover:bg-pink-50 transition-colors">
                        ...
                        <td class="px-6 py-4 whitespace-nowrap">
                            <div class="text-sm text-gray-900">
                                @if($remise->type_remise === 'pourcentage')
                                    <span class="inline-flex items-center px-2 py-1 rounded bg-pink-100 text-pink-800 text-xs font-semibold">Pourcentage</span>
                                @else
                                    <span class="inline-flex items-center px-2 py-1 rounded bg-yellow-100 text-yellow-800 text-xs font-semibold">Montant Fixe</span>
                                @endif
                            </div>
                        </td>
                        <td class="px-6 py-4 whitespace-nowrap">
                        ...
                        <td class="px-6 py-4 whitespace-nowrap">
                            <div class="text-sm text-gray-900">
                                @if($remise->date_debut && $remise->date_fin)
                        ...
                        <td class="px-6 py-4 whitespace-nowrap">
                            <form action="{{ route('admin.remises.update', $remise) }}" method="POST" class="inline">
                                @csrf
                                @method('PUT')
                        ...
                        <td class="px-6 py-4 whitespace-nowrap text-sm font-medium">
                            <div class="flex space-x-2">
                                <a href="{{ route('admin.remises.show', $remise) }}" class="text-indigo-600 hover:text-indigo-900" title="Voir détails">
                                    <i class="fas fa-eye"></i>
                                </a>
                                <a href="{{ route('admin.remises.edit', $remise) }}" class="text-blue-600 hover:text-blue-900" title="Modifier">
                                    <i class="fas fa-edit"></i>
                                </a>
                                <form action="{{ route('admin.remises.destroy', $remise) }}" method="POST" class="inline">
                                    @csrf
                                    @method('DELETE')
