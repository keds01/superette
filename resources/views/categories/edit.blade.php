    ...
    <div class="py-12">
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            <div class="bg-white/60 backdrop-blur-xl border border-indigo-100 rounded-2xl shadow-2xl p-8">
                <div class="mb-8">
                    <h2 class="text-3xl font-bold text-indigo-900">Modifier la Catégorie</h2>
                ...
                <form action="{{ route('categories.update', $categorie->id) }}" method="POST" class="space-y-6">
                    ...
                    <div class="grid grid-cols-1 gap-6 md:grid-cols-2">
                        ...
                        <div>
                            ...
                            <select name="actif" 
                                    id="actif"
                                    class="mt-1 block w-full rounded-xl border-indigo-200 focus:border-indigo-500 focus:ring focus:ring-indigo-200 focus:ring-opacity-50">
                                <option value="1" {{ old('actif', $categorie->actif) ? 'selected' : '' }}>Active</option>
                                <option value="0" {{ old('actif', $categorie->actif) ? '' : 'selected' }}>Inactive</option>
                            </select>
                            @error('actif')
                                <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                            @enderror
                    ...
                    <div>
                        ...
                        <textarea name="description" 
                                  ...
                                  required
                                  minlength="10"
                                  maxlength="500">{{ old('description', $categorie->description) }}</textarea>
                    ...
                    <div class="bg-indigo-50 rounded-xl p-4">
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-4 text-sm text-gray-600">
                            <div>
                                <p><span class="font-medium">Créée le :</span> {{ $categorie->created_at->format('d/m/Y H:i') }}</p>
                                <p><span class="font-medium">Dernière modification :</span> {{ $categorie->updated_at->format('d/m/Y H:i') }}</p>
                            </div>
                            <div>
                                <p><span class="font-medium">Nombre de produits :</span> {{ $categorie->products_count }}</p>
                                <p><span class="font-medium">Slug :</span> {{ $categorie->slug }}</p>
                            </div>
                        </div>
                    </div>
